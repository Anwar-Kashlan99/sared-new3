...
export const useWebRTC = (roomId, userDetails) => {
  ...
  const handleSetMute = useCallback(
    ...
    [setClients, userDetails._id, roomId]
  );
  ...
  const handleRejectSpeak = useCallback(({ userId }) => {
  ...
  const handleApproveSpeak = useCallback(
    ({ userId }) => {
  ...
  const handleNewPeer = useCallback(
    ...
    [addNewClient, userDetails._id, roomId]
  );
  ...
  const setRemoteMedia = useCallback(
  ...
  const getAudioLevel = useCallback(async () => {
    let audioLevel = 0.0;
  ...
  const startMonitoringAudioLevels = useCallback(() => {
  ...
  }, [getAudioLevel, isSpeaking, setClients, userDetails._id, roomId]);

  useEffect(() => {
    const initChat = async () => {
      if (socket.current) cleanupConnections();
    ...
    const setupSocketListeners = () => {
      const { current: socketInstance } = socket;
      socketInstance.on(ACTIONS.MUTE_INFO, ({ userId, isMute }) =>
        handleSetMute(isMute, userId)
      );
      socketInstance.on(ACTIONS.ADD_PEER, handleNewPeer);
      socketInstance.on(ACTIONS.REMOVE_PEER, handleRemovePeer);
      socketInstance.on(ACTIONS.ICE_CANDIDATE, handleIceCandidate);
      socketInstance.on(ACTIONS.SESSION_DESCRIPTION, setRemoteMedia);
      socketInstance.on(ACTIONS.MUTE, ({ userId }) =>
        handleSetMute(true, userId)
      );
      socketInstance.on(ACTIONS.UNMUTE, ({ userId }) =>
        handleSetMute(false, userId)
      );
      socketInstance.on(ACTIONS.ROOM_CLIENTS, ({ roomId, clients }) =>
        setClients(clients)
      );
      socketInstance.on("ROOM_ENDED_REDIRECT", handleRoomEnded);
      socketInstance.on(ACTIONS.RAISE_HAND, handleRaiseHand);
      socketInstance.on(ACTIONS.REJECT_SPEAK, handleRejectSpeak);
      socketInstance.on(ACTIONS.RAISE_HAND_DUPLICATE, ({ message }) =>
        toast(message)
      );
      socketInstance.on(ACTIONS.APPROVE_SPEAK, handleApproveSpeak);
      socketInstance.on(ACTIONS.MESSAGE, handleMessageReceived);
      socketInstance.on(ACTIONS.TALK, handleTalk);
    };

    const teardownSocketListeners = () => {
      const { current: socketInstance } = socket;
      socketInstance.off(ACTIONS.MUTE_INFO);
      socketInstance.off(ACTIONS.ADD_PEER);
      socketInstance.off(ACTIONS.REMOVE_PEER);
      socketInstance.off(ACTIONS.ICE_CANDIDATE);
      socketInstance.off(ACTIONS.SESSION_DESCRIPTION);
      socketInstance.off(ACTIONS.RAISE_HAND_DUPLICATE);
      socketInstance.off(ACTIONS.MUTE);
      socketInstance.off(ACTIONS.UNMUTE);
      socketInstance.off(ACTIONS.MESSAGE);
    };
  ...
  const raiseHand = () => {
    ...
    socket.current.emit(ACTIONS.RAISE_HAND, { roomId, ...newRequest });
